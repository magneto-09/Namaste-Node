...
app.use("/path/xyz", (req, res) =>
  res.send("still path's handler will execute")
); // subroute
...
app.use("/url9", (req, res, next) => {
  ...
  try {
    ...
    flag = 1;
    if (flag === 1)
      throw new Error("Flag is 1. It means client is not an admin.");
...
app.use("/dashboard", (req, res, next) => {
  console.log(
    "Handler that uses app.use(). Think of it as middleware for dashboard"
  );
  next();
  console.log("After next: Handler 1");
});
...
const { dummyModel: Dummy } = require("./models/dummy");
...
const { router: dummy3Router } = require("./routes/dummy3Routes");

app.use("/dummy3", dummy3Router); // dummy3 -> base path. Routes defined in dummy3Router is relative to
//  this /dummy3
